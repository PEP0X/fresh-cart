  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {isLoading ? (
        ...
        <motion.div
          className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 py-4"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        {isModalOpen && (
          <Dialog.Root open={isModalOpen} onOpenChange={setIsModalOpen}>
            ...
            <Dialog.Content
              ...
              transition={{ duration: 0.3 }}
              className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg p-6 shadow-lg sm:max-w-[425px] w-full max-h-[80vh] overflow-y-auto"
            >
              ...
              <div className="mt-4">
                {subcategories.map((subcategory, index) => (
                  <motion.div
                    ...
                    className="mb-2 p-2 bg-green-100 rounded"
                  >
                    {subcategory.name}
                  </motion.div>
                ))}
