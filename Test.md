Make Me Wishlist Component and Context Like The Cart :
    ...
    const updateCartItemQuantity = async (productId, count) => {
        try {
            let response = await axios.put(
                ...
                { count }, // Update "quantity" to "count" as per API requirements
                { headers }
            );
            return response.data;
        } catch (error) {
...
Add product to wishlits:
post => {{BaseUrl}}/api/v1/wishlist
headers : token

Remove Product from wishlist
Delete => {{BaseUrl}}/api/v1/wishlist/{productid}
headers : token
